...
La llibreria Web Server serveix per crear la paigna web, la WiFi es per configurar el WiFi, la Adafruit_NeoPixel serveix pels LEDs , la NTPClient i la WiFiUdp serveixen per configurar la hora, amb la SD i la Audio li donarem el fitxer d'audio per la alarma, i la FS és el file server:
```
#include <WebServer.h>
#include <WiFi.h>
#include <Adafruit_NeoPixel.h>
...
A continuació inciarem les variables, primer posarem les variables de la hora i de l'alarma. Les variables de l'alarma les inicialitzem a 25 la hora, i a 61 els minuts, ja que al ser valors que la hora no pot pendre no tindrem problemes amb l'alarma per defecte:
...
int posValue1; 
  ...
  int value2=61;
```
Despres afegim les variables dels colors amb els tres valors de croma RGB, l'inicialitzarem al blau:
